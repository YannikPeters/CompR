...
"))
  ...
  prepare_ldavis <- function(lda_model, text_data) {
    tryCatch({
      ...
      # Create corpus and DTM with validation
      corpus <- Corpus(VectorSource(cleaned))
      dtm <- DocumentTermMatrix(corpus)
      dtm <- dtm[rowSums(as.matrix(dtm)) > 0, ]
      if(nrow(dtm) < 5) return(NULL)
      
      # Get model parameters with NA checks
      phi <- as.matrix(posterior(lda_model)$terms)
      theta <- as.matrix(posterior(lda_model)$topics)
      if(any(is.na(phi)) || any(is.na(theta))) return(NULL)
      
      # Prepare LDAvis data with error fallback
      json <- tryCatch({
        LDAvis::createJSON(
          phi = phi,
          theta = theta,
        ...
        # Fallback to PCA if cmdscale fails
        LDAvis::createJSON(
          phi = phi,
          theta = theta,
          vocab = colnames(phi),
      ...
      message("LDAvis preparation failed: ", e$message)
      return(NULL)
    })
  }
  ...
  run_lda <- function(text_data, k) {
    ...
    # Create DTM with validation
    corpus <- Corpus(VectorSource(cleaned))
    dtm <- DocumentTermMatrix(corpus)
    dtm <- dtm[rowSums(as.matrix(dtm)) > 0, ]
    if(nrow(dtm) < 5 || ncol(dtm) < 5) return(NULL)
    
    # Run LDA with proper error handling
    tryCatch({
      LDA(dtm, k = k, control = list(seed = 123, verbose = 0))
    }, error = function(e) {
      message("LDA failed: ", e$message)
      NULL
    })
  }
  ...
  output$ldavis_output <- renderUI({
    ...
    # Validate text data
    if(nrow(dataset) < 5 || is.null(dataset$text) || all(dataset$text == "")) {
      return(div(class = "alert alert-warning",
                 "Not enough valid text data for topic modeling"))
    }
    
    withProgress(message = 'Generating visualization...', {
      lda <- run_lda(dataset$text, input$num_topics)
      if(is.null(lda)) {
        return(div(class = "alert alert-danger",
                   "Topic modeling failed - not enough meaningful text patterns"))
      }
      ...
      tags$iframe(
        srcdoc = paste0(
          '<html><head>',
          '<link rel="stylesheet" type="text/css" href="https://cdn.jsdelivr.net/gh/bmabey/pyLDAvis@1.3.1/pyLDAvis/js/ldavis.v1.0.0.css">',
          '</head><body>', 
          LDAvis::serVis(json, inbrowser = FALSE, out.dir = tempdir()), 
          '</body></html>'
        ),
        width = "100%",
        height = "600px",
  ...
  # # Render LDAvis outputs
  # output$ldavis_removed <- renderUI({
  #   req(comparison_done(), removed_posts(), input$num_topics)
  #   
  #   withProgress(message = 'Preparing removed topics visualization...', {
  #     lda <- run_lda(removed_posts()$text, input$num_topics)
  #     if(is.null(lda)) return(NULL)
  #     
  #     json <- prepare_ldavis(lda, removed_posts()$text)
  #     
  #     tags$iframe(
  #       srcdoc = paste0(
  #         '<html><head><link rel="stylesheet" type="text/css" href="https://cdn.rawgit.com/bmabey/pyLDAvis/files/ldavis.v1.0.0.css">',
  #         '<body>', LDAvis::serVis(json, inbrowser = FALSE, out.dir = tempdir()), '</body></html>'
  #       ),
  #       width = "100%",
  #       height = "600px"
  #     )
  ...
  # })
  # 
  # output$ldavis_remaining <- renderUI({
  #   req(comparison_done(), remaining_posts(), input$num_topics)
  #   
  #   withProgress(message = 'Preparing remaining topics visualization...', {
  #     lda <- run_lda(remaining_posts()$text, input$num_topics)
  #     if(is.null(lda)) return(NULL)
  #     
  #     json <- prepare_ldavis(lda, remaining_posts()$text)
  #     
  #     tags$iframe(
  #       srcdoc = paste0(
  #         '<html><head><link rel="stylesheet" type="text/css" href="https://cdn.rawgit.com/bmabey/pyLDAvis/files/ldavis.v1.0.0.css">',
  ...
  #   topic_terms <- tidytext::tidy(lda_model, matrix = "beta") %>%
  #     group_by(topic) %>%
  #     top_n(3, beta) %>%
